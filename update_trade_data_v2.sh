...
# update_trade_data_v2.sh — same workflow as update_trade_data.sh, but commits and pushes to
# https://github.com/divsum127/MindWealth_UI.git (branch: chatbot-streamlit).
# Copies all CSV files from ../MindWealth/cache/US to trade_store/stock_data (full copy).
...
# Git operations (v2: divsum127 / chatbot-streamlit)
DIVSUM_REMOTE_NAME="divsum127"
DIVSUM_REMOTE_URL="git@github.com:divsum127/MindWealth_UI.git"
DIVSUM_BRANCH="chatbot-streamlit"

if ! git remote get-url "$DIVSUM_REMOTE_NAME" &>/dev/null; then
    echo "📎 Adding git remote $DIVSUM_REMOTE_NAME → $DIVSUM_REMOTE_URL"
    git remote add "$DIVSUM_REMOTE_NAME" "$DIVSUM_REMOTE_URL"
fi
...
echo "🚀 Pushing to GitHub ($DIVSUM_REMOTE_NAME $DIVSUM_BRANCH)..."
git push "$DIVSUM_REMOTE_NAME" "HEAD:$DIVSUM_BRANCH"

echo "✅ All done! Data updated and pushed to $DIVSUM_REMOTE_NAME ($DIVSUM_BRANCH)."